...
2. 素材去重：精确哈希、基础感知指纹、相似候选组和置信度。
3. 废片尾分析：复用已验证的黑屏、静音、低纹理、静帧和重复证据。
4. 保守清理：精确裁尾、固定区域 `delogo`、掩膜 `removelogo`、可选 VSR 适配。
5. 基础翻新：显式裁切、无拉伸缩放/填充、可选调色、滤镜、降噪、锐化、镜像和同步变速。
6. 输出编码：H.264/AAC、CRF 质量模式、帧率默认保留、分辨率默认保留。
7. 计划与执行分离：`analyze`/`plan` 不生成视频，`apply` 只执行已保存计划。
8. 批处理：扩展名过滤、逐文件计划、状态持久化、断点续跑和失败清单。
9. 输出验证：文件存在、可解码、视频轨、音频轨、时长、画面尺寸、帧率和计划一致性。
